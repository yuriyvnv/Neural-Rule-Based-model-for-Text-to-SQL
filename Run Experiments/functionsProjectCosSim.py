...
SimilarityModel = SentenceTransformer("sentence-transformers/multi-qa-mpnet-base-dot-v1")
...
def question(text):
    ...
    text1= text
    global questionPrep
    print(f"Original Question: {text1}")
    questionPrep=[]
...
### EMBEDDINGS GENERATED WITH A DIFFERENT MODEL SEE IMPORTS
...
def sqlEmbeddings(names):
    ...
    embedding= SimilarityModel.encode(processedNames,convert_to_tensor=True)
    colEmbedding=SimilarityModel.encode(columnsForEmbedding,convert_to_tensor=True)
    ...
    if memory["ofCount"]==1:
        ...
        for x in goldPOS:
            if (x[0]>memory["idxfirst"]) & (x[2]in["NOUN","PatternEmbed"]) & (x[0]<=memory["interIDX"]+1):
                embedtables= SimilarityModel.encode(tables[0],convert_to_tensor=True)
                embedNoun= SimilarityModel.encode(x[1], convert_to_tensor=True)
                sim1 = util.cos_sim(embedNoun,embedtables)
                memory["idxSim1"]=x[0]-1
            if (x[0] <= memory["idxfirst"]) & (x[2] in ["NOUN","PatternEmbed"]):
                embedtables1=SimilarityModel.encode(tables[0],convert_to_tensor=True)
                embedNoun1=SimilarityModel.encode(x[1],convert_to_tensor=True)
                sim2= util.cos_sim(embedNoun1,embedtables1)
        ...
        if memory["idxAdpositionInclusive"]!=None:
            ...
            for x in goldPOS:
                if (x[0]> memory["idxAdpositionInclusive"]+1) & (x[0]<= memory["idxAdpositionInclusiveLast"]+1) & (x[2] in ["NOUN", "PROPN", "PatternEmbed"]):
                    embed= SimilarityModel.encode(x[1], convert_to_tensor=True)
                    embedTables= SimilarityModel.encode(tables[0])
                    sim1= util.cos_sim(embed, embedTables)
    ...
    try:
        if (memory["ofCount"]==1) & (memory["exceptionSimilarity"]!=1):
            if memory["ofExceptionBoth"]=='both':
                ...
                embedPattern=SimilarityModel.encode(goldPOS[memory["idxSim2"]][1], convert_to_tensor=True)
                embedColumns= SimilarityModel.encode(columns,convert_to_tensor=True)
                similarity= util.cos_sim(embedPattern, embedColumns)
            ...
            if memory["ofExceptionSecond"]=='second':
                ...
                for x in goldPOS:
                    if (x[0]<=memory["idxfirst"]) & (x[1] not in ["number"]) & (x[2] in ['NOUN','PatternEmbed']):
                        columns=tableColumns[memory["idxSim1Table"]][1:]
                        embedAllCols= SimilarityModel.encode(AllColumns, convert_to_tensor=True)
                        TabelsEmbed= SimilarityModel.encode(tables[0],convert_to_tensor=True)
                        embed= SimilarityModel.encode(x[1], convert_to_tensor=True)
                        embedTable= SimilarityModel.encode(columns, convert_to_tensor=True)
                        colSim= util.cos_sim(embed, embedAllCols)
                        column= AllColumns[torch.argmax(colSim)]
                        sim= util.cos_sim(embed, embedTable)
                        tablesSim= util.cos_sim(embed, TabelsEmbed)
            ...
            if memory["ofExceptionFirst"]=='first':
                ...
                for x in goldPOS:
                    if (x[0]<= memory["idxfirst"]) & (x[1] not in to_ignore) &(x[2] in ["NOUN", "PatternEmbed"]):  
                        embed= SimilarityModel.encode(x[1], convert_to_tensor=True)
                        EmbedTables= SimilarityModel.encode(tables[0], convert_to_tensor=True)
                        TablesSim= util.cos_sim(embed, EmbedTables)
                        ...
                        if torch.max(TablesSim)<0.8:
                            embedCols= SimilarityModel.encode(tableColumns[memory["idxSim2Table"]][1:])
                            sim = util.cos_sim(embed, embedCols)
                            ...
                            if torch.max(sim)<0.65:
                                sim2= util.cos_sim(embed, ColumnEmbeddings[0])
                                table= tables[0][torch.argmax(sim2)]
                                encodeTableCol= SimilarityModel.encode(tableColumns[table], convert_to_tensor=True)
                                sim3= util.cos_sim(embed,encodeTableCol) 
                    ...
                    if (x[0]>= memory["idxfirst"]) & (x[1] not in to_ignore) &(x[2] in ["NOUN", "PatternEmbed"]):
                        ...
                        if torch.max(TablesSim)<0.8:
                            embedCols= SimilarityModel.encode(tableColumns[memory["idxSim1Table"]][1:])
                            sim = util.cos_sim(embed, embedCols)
                            if torch.max(sim)>=0.65:
                                goldPOS[x[0]-1]= [x[0], f'Table:{memory["idxSim1Table"]}', f'Column:{tableColumns[memory["idxSim1Table"]][1:][torch.argmax(sim)]}']
                            if torch.max(sim)<0.65:
    ...
    try:
        if(memory["ofCount"]==1) & (memory["exceptionSimilarity"]!=1)  :
            if memory["ofExceptionNeither"]=='neither':
                embed= SimilarityModel.encode(goldPOS[memory["idxSim1"]][1], convert_to_tensor=True)
                embedTables= SimilarityModel.encode(tables[0], convert_to_tensor=True)
                embedALLCols= SimilarityModel.encode(AllColumns, convert_to_tensor=True)
                tablesim= util.cos_sim(embed,embedTables)
                AllColsSim= util.cos_sim(embed, embedALLCols)
                if (torch.max(tablesim)>0.55) & (torch.max(AllColsSim)<0.70):
                    embedColumns = SimilarityModel.encode(tableColumns[tables[0][torch.argmax(tablesim)]][1:])
                    colsim= util.cos_sim(embed, embedColumns)
                    ...
                    for x in goldPOS:
                        if (x[0]<= memory["idxfirst"]) & (x[2]in ["NOUN", "PatternEmbed"]) & (x[1] not in to_ignore):
                            embed=SimilarityModel.encode(x[1],convert_to_tensor=True)
                            columnEmbed= SimilarityModel.encode(columns, convert_to_tensor=True)
                            sim = util.cos_sim(embed, columnEmbed)
                            TablesSim= util.cos_sim(embed, embedTables)
                            ...
                            if (torch.max(sim)<=0.6) & (torch.max(TablesSim)<0.85):
                                allCollEmbed= SimilarityModel.encode(AllColumns, convert_to_tensor=True)
                                simCol= util.cos_sim(allCollEmbed, embed)
                ...
                if (torch.max(tablesim)<=0.55):
                    ...
                    if (memory["idxAdpositionInclusiveLast"]!=None) & (memory["idxAdpositionInclusive"]!=None):
                        for x in goldPOS:
                            if (x[0]>=memory["idxAdpositionInclusive"]+1) & (x[0]<=memory["idxAdpositionInclusiveLast"]+1) & (x[2] in ["NOUN", "PatternEmbed"]) & (x[1] not in to_ignore):
                                embedPattern= SimilarityModel.encode(x[1], convert_to_tensor=True)
                                embedTable= SimilarityModel.encode(tables[0],convert_to_tensor=True)
                                #### add similarity to search table, if big enough add main table after of, if not search for column 
                                TableSim= util.cos_sim(embedPattern, embedTable)
                                if torch.max(TableSim)>=0.85:
                                    goldPOS[x[0]-1]= [x[0], f"Table:{tables[0][torch.argmax(TableSim)]}", "Main Table after 'of"]
                                    memory["idxSim1Table"]= tables[0][torch.argmax(TableSim)]
                                if torch.max(TableSim)<0.85:
                                    allColumnEmbedding= SimilarityModel.encode(AllColumns)
                                    colsim= util.cos_sim(embedPattern, allColumnEmbedding)
                        ...
                        for x in goldPOS:
                            if (x[0]<= memory["idxAdpositionInclusive"]+1)& (x[0]>= memory["idxfirst"]+1) &(x[2]in ["NOUN", "PatternEmbed"]) & (x[1] not in to_ignore):
                                ...
                                embedCols= SimilarityModel.encode(tableColumns[tables[0][torch.argmax(TableSim)]][1:])
                                ColSim= util.cos_sim(embedPattern, embedCols)
                                ...
                                if torch.max(ColSim)<0.65:
                                    TableSim= util.cos_sim(embedPattern, tableColumnEmbeddings[0])
                                    ...
                                    embedTableCols= SimilarityModel.encode(tableCols)
                                    exceptSim= util.cos_sim(embedPattern, embedTableCols)
                    ...
                    if (memory["idxAdpositionInclusiveLast"]==None) & (memory["idxAdpositionInclusive"]== None):
                        allCollEmbed= SimilarityModel.encode(AllColumns, convert_to_tensor=True)
                        colsim= util.cos_sim(embed, allCollEmbed)
                        if torch.max(colsim)>0.4:
                            column= AllColumns[torch.argmax(colsim)]
                            table= [key for key, value_list in tableColumns.items() if column in value_list][0]
        ...
        if (memory["ofCount"]==1) & (memory["ofExceptionNeither"]== "neither") & (memory["exceptionSimilarity"]==1):
            ...
            for x in goldPOS:
                if (x[0]>= memory["idxSim1"]) & (x[0]<= memory["interOfidx"]+1) & (x[2] in ["NOUN", "PatternEmbed"]) & (x[1] not in to_ignore ):
                    embedTables= SimilarityModel.encode(tables[0], convert_to_tensor=True)
                    embed= SimilarityModel.encode(x[1], convert_to_tensor=True)
                    if torch.max(util.cos_sim(embed, embedTables))>=0.8:
                        goldPOS[x[0]-1]= [x[0], f'Table:{tables[0][torch.argmax(util.cos_sim(embed, embedTables))]}', "Reference Table after 'of"]
                        memory["idxSim1Table"]= tables[0][torch.argmax(util.cos_sim(embed, embedTables))]
                    if torch.max(util.cos_sim(embed, embedTables))<0.8:
                        colEmbeddings= SimilarityModel.encode(AllColumns, convert_to_tensor=True)
                        ColSim= util.cos_sim(embed, colEmbeddings)
                        column= AllColumns[torch.argmax(ColSim)]
                        table= [key for key, value_list in tableColumns.items() if column in value_list][0]
            ...
            for x in goldPOS:
                if (x[0]<=memory["idxSim1"])&(x[2] in ["NOUN","PROPN", "PatternEmbed"]) &(x[1] not in to_ignore):
                    ...
                    embed= SimilarityModel.encode(x[1], convert_to_tensor=True)
                    if torch.max(util.cos_sim(embed, embedTables))>=0.8:
                        goldPOS[x[0]-1]= [x[0], f'Table:{tables[0][torch.argmax(util.cos_sim(embed, embedTables))]}', "Reference Table before 'of"]
                        memory["idxSim1Table"]= tables[0][torch.argmax(util.cos_sim(embed, embedTables))]
                    if torch.max(util.cos_sim(embed, embedTables))<0.8:
                        colEmbeddings= SimilarityModel.encode(AllColumns, convert_to_tensor=True)
                        ColSim= util.cos_sim(embed, colEmbeddings)
    ...
    try:
        if memory["ofCount"]>=2:
            ...
            for x in goldPOS:
                if (x[2] in ["NOUN", "PatternEmbed"]) & (x[0]> memory["ofIdxSecond"]) & (x[1] not in to_ignore) & (x[0]<= memory["interIDXSecondOF"]+1):
                    embed= SimilarityModel.encode(x[1], convert_to_tensor=True)
                    tablesEmbed= SimilarityModel.encode(tables[0], convert_to_tensor=True)
                    sim= util.cos_sim(embed, tablesEmbed)
                    if torch.max(sim)>0.85:
                        ...
                        for z in goldPOS:
                            if (z[0]<= memory["interIDXdoubleOF"]) & (z[0]>memory["idxfirst"]) & (z[2] in ["NOUN", "PatternEmbed"]) & (z[1] not in to_ignore):
                                embed1= SimilarityModel.encode(z[1], convert_to_tensor=True)
                                tablesEmbed= SimilarityModel.encode(tables[0], convert_to_tensor=True)
                                sim1 = util.cos_sim(embed1, tablesEmbed)
                                ...
                                if torch.max(sim1)<0.75:
                                    ...
                                    colEmbeddings= SimilarityModel.encode(AllColumns, convert_to_tensor=True)
                                    ColSim= util.cos_sim(embed1, colEmbeddings)
                                    column= AllColumns[torch.argmax(ColSim)]
                                    table= [key for key, value_list in tableColumns.items() if column in value_list][0]
                                    memory["idxSim2Table"]= table
                                    embedMainTableCols= SimilarityModel.encode(tableColumns[tables[0][torch.argmax(sim)]][1:])
                                    mainTableColSimilarity= util.cos_sim(embed1, embedMainTableCols)
        ...
        for x in goldPOS:
            if (x[0]<= memory["ofIdxSecond"]) & (x[2] in ["NOUN", "PatternEmbed"]) & (x[1] not in to_ignore):
                embed= SimilarityModel.encode(x[1])
                columns= tableColumns[tables[0][torch.argmax(sim1)]]
                embedTable= SimilarityModel.encode(columns)
                sim2 = util.cos_sim(embed, embedTable)
                if torch.max(sim2)>=0.6:
                    goldPOS[x[0]-1]= [x[0], f"Table:{tables[0][torch.argmax(sim1)]}", f"Column:{columns[torch.argmax(sim2)]}"]
                if torch.max(sim2)<0.6:
                    sim3= util.cos_sim(embed, tableColumnEmbeddings[0])
                    table= tables[0][torch.argmax(sim3)]
                    embedCol= SimilarityModel.encode(tableColumns[table], convert_to_tensor=True)
                    sim4= util.cos_sim(embed, embedCol)
            ...
            if memory["mainTableAfterDoubleOf"]!= None:
                for x in goldPOS:
                    if (x[0] >= memory["idxfirst"]+1) & (x[0]<= memory["ofIdxSecond"]) & (x[2] in ["NOUN", "PatternEmbed"]) & (x[1] not in ["number","different", "distinct","order"]): 
                        ...
                        embedColumns= SimilarityModel.encode(columns, convert_to_tensor=True)
                        sim1= util.cos_sim(embed, embedColumns)
                ...
                for x in goldPOS: 
                    if (x[0]<= memory["idxfirst"]) & (x[2] in ["NOUN", "PatternEmbed"]) & (x[1] not in ["number","different", "distinct","order"]):
                        embed= SimilarityModel.encode(x[1],convert_to_tensor=True)
                        TablesEmbed= SimilarityModel.encode(tables[0], convert_to_tensor=True)
                        embedTableCol= SimilarityModel.encode(tableColumns[memory["mainTableAfterDoubleOf"]][1:])
                        sim= util.cos_sim(embed, embedTableCol)
                        tableSim= util.cos_sim(embed,TablesEmbed)
                        if torch.max(tableSim)>=0.85:
                        ...
                        if (torch.max(sim)>0.65) & (torch.max(tableSim)<0.85):
                            ...
                            sim1= util.cos_sim(embed, tableColumnEmbeddings[0])
                            columns= tableColumns[tables[0][torch.argmax(sim1)]][1:]
                            embedCol= SimilarityModel.encode(columns)
                            sim2= util.cos_sim(embed, embedCol)
    ...
    try:
        ...
        for x in goldPOS: 
            if (x[0]>memory["idxfirst"]) & (x[0]<= memory["interIDXdoubleOF"]) & (x[1] not in to_ignore) & (x[2] in ["NOUN", "PatternEmbed"]):
                embed= SimilarityModel.encode(x[1], convert_to_tensor=True)
                embedTables= SimilarityModel.encode(tables[0], convert_to_tensor=True)
                sim= util.cos_sim(embed, embedTables)
        ...
        if memory["mainTableBetweenDoubleOf"]==1:
            for x in goldPOS:
                if (x[0]>=memory["interIDXdoubleOF"]) & (x[0]<=memory["interIDXSecondOF"]) & (x[1] not in to_ignore) & (x[2] in ["NOUN", "PatternEmbed"]):
                    embed= SimilarityModel.encode(x[1], convert_to_tensor=True)
                    embedCols= SimilarityModel.encode(tableColumns[memory["idxSim1Table"]][1:], convert_to_tensor=True)
                    sim2= util.cos_sim(embed, embedCols)
                    ...
                    if (torch.max(sim2)<0.55) & (torch.max(sim2)>0.40):
                        sim3= util.cos_sim(embed, ColumnEmbeddings[0])
                        differentTable= tables[0][torch.argmax(sim3)]
                        embedCols= SimilarityModel.encode(tableColumns[differentTable][1:], convert_to_tensor=True)
                        sim4= util.cos_sim(embed, embedCols)
                        if (x[2]== "PatternEmbed") & (torch.max(sim3)>=0.6):
                            splitted_string= x[1].split("_")
                            first= SimilarityModel.encode(splitted_string[0], convert_to_tensor=True)
                            last= SimilarityModel.encode(splitted_string[-1], convert_to_tensor=True)
                            simFirst= util.cos_sim(first, embedCols)
                            simLast= util.cos_sim(last, embedCols)
        ...
        try:
            for x in goldPOS:
                if (x[0]<= memory["idxfirst"])  & (x[1] not in to_ignore) & (x[2] in ["NOUN", "PatternEmbed"]):
                    ...
                    embedAllCols= SimilarityModel.encode(AllColumns, convert_to_tensor= True)
                    embedTables = SimilarityModel.encode(tables[0], convert_to_tensor=True)
                    simTable= util.cos_sim(embed, embedTables)
                    sim1= util.cos_sim(embed, embedAllCols)
            ...
            if x[1] in special_cases:
                if x[1] in ["older"]:
                    ...
                    if memory["idxSim1Table"]!=None:
                        ...
                        embedTableCols= SimilarityModel.encode(tableCols, convert_to_tensor=True)
                        sim1 = util.cos_sim(embed, embedTableCols)
                        ...
                        if torch.max(sim1)<0.45:
                            embedAllCols= SimilarityModel.encode(AllColumns, convert_to_tensor=True)
                            sim2 =util.cos_sim(embed, embedAllCols)
    ...
    if memory["ofCount"]>0:
        for x in goldPOS:
            ...
            if (x[0]>memory["idxfirst"]) & (x[2]== "NUM") & (len(x[1])==4):
                ...
                if match != None:
                    num = match.group(1)
                    embed = SimilarityModel.encode(num, convert_to_tensor=True)
                    embedAllCols= SimilarityModel.encode(AllColumns,  convert_to_tensor=True)
                    sim = util.cos_sim(embed, embedAllCols)
            ...
            for x in goldPOS:
                if ((x[0]-1)>=index ) & (x[2] in ["NOUN", "PatternEmbed"]) & (x[1] not in to_ignore):
                    ...
                    if goldPOS[index][1] not in ["AVG", "SUM"]: 
                        break
                    embedingOriginalWord= SimilarityModel.encode(x[1], convert_to_tensor=True)
                    embedNewWord=SimilarityModel.encode(new_word, convert_to_tensor=True)
                    embedAllcolumns= SimilarityModel.encode(AllColumns, convert_to_tensor=True)
                    simOriginal= util.cos_sim(embedingOriginalWord, embedAllcolumns)
                    simNewWord= util.cos_sim(embedNewWord, embedAllcolumns)
                    if torch.max(simOriginal)> torch.max(simNewWord) :
                        pass
                    if torch.max(simNewWord)> torch.max(simOriginal):
                        embedNewWord2= SimilarityModel.encode(new_word2)
                        simNewWord2= util.cos_sim(embedAllcolumns, embedNewWord2)
                ...
                if ((x[0]-1)>= index ) & (x[2].startswith("Column:")):
                    ...
                    embedingOriginalWord= SimilarityModel.encode(originalWord, convert_to_tensor=True)
                    embedingNewWord= SimilarityModel.encode(new_word, convert_to_tensor=True)
                    simOriginalWord= util.cos_sim(embedingOriginalWord, embedAllcolumns)
                    simNewWord= util.cos_sim(embedingNewWord, embedAllcolumns)
            ...
            for x in goldPOS:
                if (x[0]>memory["idxfirst"]) & (x[2] in ["NOUN", "PatternEmbed"])  & (x[1] not in to_ignore) :
                    ...
                    if memory["idxSim1Table"]!=None:
                        ...
                        embedCols= SimilarityModel.encode(cols, convert_to_tensor=True)
                        sim1 = util.cos_sim(embed, embedCols)
                        allColumnsEmbeding= SimilarityModel.encode(AllColumns, convert_to_tensor=True)
                        colSim= util.cos_sim(embed, allColumnsEmbeding)
            ...
            for x in goldPOS:
                if (x[0]>memory["ofIdxSecond"]) & (x[2] in ["PatternEmbed", "NOUN"])&(x[1] not in to_ignore):
                    try:
                        ...
                        sim= util.cos_sim(embed, embedCols)
                        embedAllcols= SimilarityModel.encode(AllColumns, convert_to_tensor=True)
                        simAllCols= util.cos_sim(embed, embedAllcols)
                        ...
                        if (torch.max(sim)<0.75) & ((torch.max(simAllCols)<=0.75)):
                            ...
                            if torch.max(TableSim)<0.75:
                                AllColsEmbeding=SimilarityModel.encode(AllColumns, convert_to_tensor=True)
                                colSim= util.cos_sim(embed, AllColsEmbeding)
                                if torch.max(colSim)>0.4:
                                    column = AllColumns[torch.argmax(colSim)]
                                    table= [key for key, value_list in tableColumns.items() if column in value_list][0]
                                    goldPOS[x[0]-1]= [x[0], f'Table:{table}', f"Column:{column}"]
            ...
            if (internalFlagValue==True) & (internalFlagColumn==False):
                    for z in goldPOS:
                        if (z[0]<= validx) &( z[0] > memory["idxfirst"]) & (z[1] in notIN):
                            func= "!="
                    if memory["idxSim1Table"]!= None:
                        embed = SimilarityModel.encode(val, convert_to_tensor=True)
                        embedMainTable = SimilarityModel.encode(tableColumns[memory["idxSim1Table"]][1:], convert_to_tensor=True)
                        sim= util.cos_sim(embed, embedMainTable)
                        if torch.max(sim)>0.3:
                            table = memory["idxSim1Table"]
                            column = tableColumns[memory["idxSim1Table"]][1:][torch.argmax(sim)]
                            where= f"{table}.{column}"
                            if func== None:
                                memory["WhereClause"].append(f"{where}")
                    ...
                    if memory["idxSim1Table"]== None:
                        embed = SimilarityModel.encode(val, convert_to_tensor=True)
                        embedAllColumns= SimilarityModel.encode(AllColumns,convert_to_tensor=True)
                        sim= util.cos_sim(embed, embedAllColumns)
        ...
        if flag == False:
            ...
            for x in goldPOS:
                if (x[0]>memory["idxfirst"]) & (x[2]in ["propnPattern"]):
                    ...
                    if memory["idxSim1Table"]!= None:
                        embed= SimilarityModel.encode(x[1], convert_to_tensor=True)
                        embedAll= SimilarityModel.encode(tableColumns[memory["idxSim1Table"]][1:],convert_to_tensor=True)
                        sim =util.cos_sim(embed, embedAll)
                        if torch.max(sim)>=0.4:
                    ...
                    if (memory["idxSim1Table"]== None) | (internalFlag==True):
                        ...
                        embedTableCols= SimilarityModel.encode(tableColumns[table][1:],convert_to_tensor=True)
                        sim =  util.cos_sim(embed, embedTableCols)
...
def joinTables(goldPOS, memory):
    ...
    if (memory["ofCount"]>0) & (len(set(memory["Tables"]))>1):
        ...
        if len(AllIDS)>2:
            embedAllIDS= SimilarityModel.encode(AllIDS, convert_to_tensor=True)
            embedIdsToJoin= SimilarityModel.encode([list(idsToJoin.values())], convert_to_tensor=True)
            simIDS= util.cos_sim(embedIdsToJoin, embedAllIDS)
            specificIDembed= SimilarityModel.encode(AllIDS[torch.argmax(simIDS)], convert_to_tensor=True)
            keyTable = None
            for key, value in ids.items():
            ...
            for x in idsToJoin:
                tableIDS= {}
                embed= SimilarityModel.encode(idsToJoin[x], convert_to_tensor=True)
                sim= util.cos_sim(embed, specificIDembed)
    ...
    if (memory["ofCount"]==0):
        ...
        for x in goldPOS:
            if (x[0] > memory["phraseBroke"]) & (x[2] in ["NOUN", "PatternEmbed"]) &(x[1] not in to_ignore):
                embed = SimilarityModel.encode(x[1])
                embedTables = SimilarityModel.encode((list(tableColumns.keys())))
                simTable= util.cos_sim(embed,embedTables)
                embedAllcols= SimilarityModel.encode(AllColumns)
                simAllcols= util.cos_sim(embed, embedAllcols)
                ...
                if (memory["MainTableAfterBreak"]!= None) &(x[0]-1 != memory["idxMainTableAfterBreak"]) :
                    embedTableCols= SimilarityModel.encode(tableColumns[memory["MainTableAfterBreak"]])
                    sim1 = util.cos_sim(embed, embedTableCols)
                    if torch.max(sim1)>=0.75:
    ...
    if (memory["ofCount"]==0):
        for x in goldPOS:
            if (x[0]<=memory["phraseBroke"]) & (x[2] in ["NOUN", "PatternEmbed"]) & (x[1] not in to_ignore):
                ...
                simAllcols= util.cos_sim(embed, embedAllcols)
                if memory["MainTableAfterBreak"] != None:
                    embedTableCols= SimilarityModel.encode(tableColumns[memory["MainTableAfterBreak"]][1:], convert_to_tensor=True)
                    sim1 = util.cos_sim(embed,embedTableCols)
    ...
    if memory["ofCount"]==0:
        ...
        if len(memory["WhereClause"])==1:
            ...
            if (internalFlagValue==True) & (internalFlagColumn==False):
                    ...
                    if memory["MainTableAfterBreak"]!= None:
                        embed = SimilarityModel.encode(val, convert_to_tensor=True)
                        embedMainTable = SimilarityModel.encode(tableColumns[memory["MainTableAfterBreak"]][1:], convert_to_tensor=True)
                        sim= util.cos_sim(embed, embedMainTable)
                        if torch.max(sim)>0.3:
                            table = memory["MainTableAfterBreak"]
                            column = tableColumns[memory["MainTableAfterBreak"]][1:][torch.argmax(sim)]
                            where= f"{table}.{column}"
                            if func== None:
                                memory["WhereClause"].append(f"{where}")
                                memory["WhereClause"].append(f" = '{val}' ")
    ...
    if memory["ofCount"]==0:
        ...
        if flag == False:
            for x in goldPOS:
                if (x[0]>memory["phraseBroke"]) & (x[2]in ["propnPattern"]):
                        embed= SimilarityModel.encode(x[1], convert_to_tensor=True)
                        embedAll= SimilarityModel.encode(AllColumns,convert_to_tensor=True)
                        sim =util.cos_sim(embed, embedAll)
                        column = AllColumns[torch.argmax(sim)]
                        table= [key for key, value_list in tableColumns.items() if column in value_list][0]
                        memory["WhereClause"].append(f"""{table}.{column}""")
...
datasetTotest= pd.read_csv("/path/toTheData/")
    ...
    if len(sql)>0:
        if sql != "Gave error":
            ...
            try:
                ...
                del df
                del dforiginal
            except:print("erro  de validação")
end = time.time()
print(end - start)

print(i)
test_results.to_csv("devALLFirstModelCosineSim.csv")
